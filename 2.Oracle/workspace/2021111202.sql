2021-1112-02)
 ...
 사용예) SELECT SYSDATE, SYSDATE+20, SYSDATE-20, SYSDATE-TO_DATE('20201112'),
                ROUND(SYSDATE-TO_DATE('20201112'),0), 
                TRUNC (SYSDATE-TO_DATE('20201112')) 
        FROM DUAL;
        
  2) ADD_MONTHS (d,n)
   - 주어진 날짜자료 d에 n만큼의 월을 더한 날자 반환
 ...
 사용예) 매입상품에 따른 결제일은 매입일 기준 2개월 후 이다. 2005년 3월 일자별 결제할금액을 조회하시오.
        Alias는 날짜, 결제금액이다. 
  ...
  3) NEXT_DAY(d, fmt), LAST_DAY(d)
   - NEXT_DAY : 주어진 날짜 d 이후에 처음 만나는 'fmt'로 시술된 요일의 날짜 반환.
                'fnt' 는 요일로 '월', '월요일',..으로 기술
 ...
 사용예) 회원들의 정확한 나이를 조회하시오.
        ...
        SELECT MEM_ID AS 회원번호,
                ...
                MOD(ROUND(MONTHS_BETWEEN(SYSDATE,MEM_BIR)), 12)||'M' AS 나이
        FROM MEMBER;
        
  5) EXTRACT(fmt FROM d)
   - 날짜자료 d에 포함된 각 요소 (fmt : 년, 월, 일, 시, 분, 초) 값을 반환
   - fmt 는 YEAR, MONTH, DAY, HOUR, MINUTE, SECOND
