2021-1123-01) 테이블 조인
 ...
 - 관계(Relationship : 외래키 관계)를 형성하고 있는 복수개의 테이블에서 필요한 자료를 추출하기 위한 연산
 - 내부조인(카타시안조인, 동등조인, 비동등조인, 셀프조인), 외부조인
 -- 조인조건에서 = 이 사용되면 동등조인, !=이 사용되면 비동등조인
 - 일반조인, ANSI조인(CROSS JOIN, NATURAL JOIN, INNER JOIN, OUTER JOIN)
 -- 조인조건을 만족하는 애들만 가지고 연산을 수행하는게 내부조인(이너조인)
 ...
 - 조인에 참여하는 테이블들 사이에 조인조건을 만족하지 않는 모든 행을 무시하고 연산
  ...
  SELECT [테이블명, [테이블별칭.]컬럼명 [AS 별칭][,] 
                        ...
                        .
                        .
  FROM 테이블명 [별칭1], 테이블명2 [별칭2][,테이블명3 [별칭3],....]
  WHERE [일반조건]
  AND 조인조건1
  [AND 조인조건2,....]
  
  . 일반조건과 조인조건의 기술 순서는 제한없음
  . 사용된 테이블의 갯수가 n개일 때, 조인조건은 적어도 n-1개 이상 이어야함
  . 조인조건이 잘못 기술되거나 생략되면 카타시안 PRODUCT 로 수행됨
  . 조인조건은 사용된 테이블들의 공통된 컬럼을 관계연산자를 사용하여 비교
  
 1) Cartesian Product
  - 다수개의 테이블에서 모든 행, 모든 열이 조합됨
  - 행의 조합은 곱이되고 열의 조합은 합이됨
  - 아주 특별한 목적이외에는 사용하지 말아야 함
  - 조인조건이 없거나 조인조건이 잘못 기술된 경우 발생
  - ANSI는 CROSS JOIN 으로 구현
 ...
 2) 동등조인(Eai-join)
  - 조인조건에 '=' 연산자를 사용한 조인
  - 조인조건을 만족하는 자료만을 대상으로 연산
  - ANSI 조인에서 INNER JOIN 이 이에 해당
  ...
  (ANSI INNER JOIN 사용형식)
        ...
        -- 조인 결합 방식 : 테이블1 과 테이블2의 조인 후, 그 결과값과 테이블3가 조인된다.
        -- 조인절의 일반조건은 조인을 맺는 두 테이블의 조건을 나타낸다. 
        -- 예) 일반조건1 = 테이블1 과 테이블2의 공통조건
                    .
                    .
        [WHERE 일반조건]
        
 사용예) 2005년 7월 매출자료를 조회하시오
        ...
        FROM CART A, PROD B
        WHERE SUBSTR(A.CART_NO,1,8) LIKE '200507%'  --일반조건
        AND A.CART_PROD = B.PROD_ID  --조인조건(상품명과 판매단가를 참조하기 위해서)
        ORDER BY 1;
        
        SELECT
        FROM CART A, PROD B
        
    -- 기준이 되는 테이블과 아닌 테이블을 구별(테이블 내에서 해결할 수 있는가 와 없는가?)
    -- 각각의 테이블을 찾는다.
    -- 찾은 테이블이 공통의 테이블이 있는가 없는가를 찾는다.
        
                
        
 사용예) 사원테이블에서 급여가 15000 이상 되는 사원정보를 조회하시오
        ...
        AND A.DEPARTMENT_ID = B.DEPARTMENT_ID --조인조건(부서명 참조)
        AND A.JOB_ID = C.JOB_ID -- 조인조건(직무명 참조)
        ORDER BY 1;
 ...
 사용예) 2005년 2월 거래처별 매입집계를 조회하시오
        ...
        SELECT  B.BUYER_ID AS 거래처코드,
                ...
                SUM(A.BUY_QTY * C.PROD_COST) AS 매입급액합계
